...
SwaggerUi.Views.OperationDataView = Backbone.View.extend({
	invocationUrl: null,

	events: {
		'submit .sandbox'         : 'submitOperation',
		'click .submit'           : 'submitOperation',
		'click .toggleTry .tab'        : 'toggleTryOperation',
	...
	render: function() {
		...
		// console.log(3,this.model.path, this.model.parameters, signatureModel)
		$(this.el).html(Handlebars.templates.operation_data(this.model));
	...
	addParameter: function(param, consumes) {
		...
		var tryParamView = new SwaggerUi.Views.TryApiView({
			model: param,
			tagName: 'tr',
			readOnly: this.model.isReadOnly,
			swaggerOptions: this.options.swaggerOptions
		});
		$('.ta-operation-params', $(this.el)).append(tryParamView.render().el);
	},
	...
	toggleTryOperation: function(event) {
		const elemClick = $(event.currentTarget);
		// const child = $(elemClick.children()[0]);

		if (!elemClick.hasClass('active')) {
			elemClick.addClass('active');
			elemClick.siblings().removeClass('active');
			$('.response-class', $(this.el)).toggle();
			$('.try-api', $(this.el)).toggle();
		}
	}
